...
const MemberItem = ({ name, role, image, memberId, mobile }) => {
  return (
    <div className="overflow-hidden relative w-full h-full">
      ...
      <div className="w-full z-50 absolute bottom-2 opacity-0 group-hover:opacity-100 transition duration-500 ease-in-out p-2 text-white">
        <div className="flex justify-between items-end px-3 w-full">
          <div>
            <div className="font-brand text-lg md:text-xl lg:text-2xl">
              {name}
            </div>

            <div className="font-poppins tracking-[0.25rem] text-xs font-extralight my-2">
              {role}
            </div>
            <div className="font-poppins tracking-[0.25rem] text-xs font-extralight my-2">
              {memberId}
